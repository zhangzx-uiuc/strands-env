...
"""
Agentless generation function for algorithmic Python coding RL training.

Uses direct one-shot generation without tool interaction, then evaluates
the generated code using CodeTestCaseReward.
"""
...
from strands_env.core.types import Action, Observation, RewardResult, StepResult, TaskContext, TerminationReason
...
Guidelines:
- Carefully analyze the problem and consider edge cases
- Provide your final solution in a ```python code block
...
async def generate_and_rm(args, sample: Sample, sampling_params) -> Sample:
    ...
    reward_result = await reward_fn.compute(action, step_result)
    elapsed = time.perf_counter() - start
    logger.info("reward_fn.compute took %.3fs", elapsed)
    sample.reward = reward_result.reward
    sample.metadata["reward_info"] = reward_result.info
